...
* 본인의 직전학기 수강 신청 학점과 학점을 입력해 국가장학금 수혜 가능 여부를 판단하고  본인과 가족의 월 소득액, 일용 소득액 , 일반 재   산, 금융재산, 차량가격을 입출력 함수를 이용해 입력받아 국가 장학금이 인정하는 비율을 곱해 모든 함수를 다 더해 총  월 소득인정액을     계산해 변수나 포인터를 통해 저장해둡니다. 산출된 소득과 가구원수를 입력받아 학자금 지원액 판단에 필요한 소득분위를 계산 합니다 니다.
